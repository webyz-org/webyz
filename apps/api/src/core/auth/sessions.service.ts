...
// The cache is an accelerator for the Postgres `sessions` table, so every
// Redis failure below is treated as a miss (or a no-op write): with Redis
// down a dashboard keeps working from the database instead of every
// authenticated request failing.

async function getCachedSession(
  redis: Redis,
  sessionId: string,
): Promise<CachedSession | null> {
  const raw = await redis.get(cacheKey(sessionId)).catch(() => null);
  if (!raw) return null;
  try {
...
async function setCachedSession(
  ...
  await redis
    .setex(cacheKey(session.sessionId), CACHE_TTL, JSON.stringify(session))
    .catch(() => {});
}
...
async function invalidateCachedSession(
  ...
  await redis.del(cacheKey(sessionId)).catch(() => {});
}
...
async function invalidateAllUserSessions(
  ...
  await redis.del(...sessionIds.map(cacheKey)).catch(() => {});
}
