...
| `REFERRER_SPAM_LISTS` | the Matomo referrer spam list | Domains whose fabricated referrals exist only to appear in your Sources card. A request whose referrer is one of them, or a subdomain, is dropped. Same job, `apps/api/geo/referrer-spam.txt`; `off` disables it. For every list variable an empty value means the default, because the compose files pass unset variables through as empty strings |
| `BOT_CLUSTER_FILTER` | `on` | Behavioural filter. Every five minutes the `detect-scripted-traffic` job looks at the last hour of sessions per site, grouped by screen size, browser and language, and flags a group of at least 50 visits from at least 20 visitors when 95% or more are single-pageview visits and not one reported engagement (people switch tabs and scroll; scripts do not). Requests matching a flagged group are dropped for 24 hours. A site is only examined once at least 30 of its sessions in that hour carry engagement, which proves its tracker is current, so a site still on an older cached tracker is never judged. `off` disables detection and the check |
...
| `IMAGE_REGISTRY`, `IMAGE_TAG` | Where prebuilt images come from (`ghcr.io/webyz-org`) and which tag (`latest` or a version) |
